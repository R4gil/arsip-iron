...
<div id="notifikasi-box" class="notifikasi-box me-2">
    <a href="#" class="notifikasi-toggle" data-bs-toggle="dropdown" aria-expanded="false" title="Notifikasi">
        ...
        <?php if($notifikasiCount > 0): ?>
            <span class="notifikasi-badge" id="notifikasi-badge"><?php echo e($notifikasiCount > 9 ? '9+' : $notifikasiCount); ?></span>
        <?php else: ?>
        ...
        <div class="notifikasi-list" id="notifikasi-list">
            <?php $__empty_1 = true; $__currentLoopData = $notifikasiList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notif): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <a href="<?php echo e($notif['url']); ?>"
                   ...
                   data-key="<?php echo e($notif['key']); ?>">
                    <span class="notifikasi-item-icon" style="background: <?php echo e($notif['warna']); ?>15; color: <?php echo e($notif['warna']); ?>;">
                        <i class="fas <?php echo e($notif['ikon']); ?>"></i>
                    </span>
                    <span class="notifikasi-item-body">
                        <span class="notifikasi-item-title"><?php echo e($notif['judul']); ?></span>
                        <span class="notifikasi-item-message"><?php echo e($notif['pesan']); ?></span>
                        <span class="notifikasi-item-time"><?php echo e($notif['waktu_label']); ?></span>
                    </span>
                </a>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function dismissNotification(key, element) {
        if (!key) return;
        try {
            const response = await fetch('<?php echo e(route('notifikasi.dismiss')); ?>', {
                method: 'POST',
                headers: {
    ...
    if (dismissAllBtn) {
        dismissAllBtn.addEventListener('click', async function (e) {
            ...
            try {
                await fetch('<?php echo e(route('notifikasi.dismissAll')); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrf,
...
<?php /**PATH C:\laragon\www\iron-smart\resources\views/partials/header-notifications.blade.php ENDPATH**/ ?>