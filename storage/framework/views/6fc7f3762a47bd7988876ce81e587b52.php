...
<div class="row g-3 mb-4">
    <?php $__currentLoopData = [['Total', $stats['total'], 'fa-list'], ['Dipinjam', $stats['dipinjam'], 'fa-book'], ['Dikembalikan', $stats['dikembalikan'], 'fa-check']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="col-md-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius: 12px;">
            <div class="card-body d-flex align-items-center gap-3 p-4">
                <div style="background: #f8fafc; color: #475569; width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem;"><i class="fas <?php echo e($s[2]); ?>"></i></div>
                <div><div style="color: #64748b; font-size: 0.8rem; font-weight: 600;"><?php echo e($s[0]); ?></div><div style="font-weight: 700; font-size: 1.5rem; color: #1e293b;"><?php echo e($s[1]); ?></div></div>
            </div>
        </div>
    </div>
...
<div class="card border-0 shadow-sm mb-3" style="border-radius: 12px;">
    <div class="card-body p-3">
        <form method="GET" action="<?php echo e(route('peminjaman.index')); ?>" class="row g-2 align-items-end">
            <div class="col-md-5">
                <label class="form-label fw-semibold" style="color: #334155; font-size: 0.85rem;">Cari</label>
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" class="form-control form-control-sm" placeholder="Peminjam, nomor/nama arsip..." style="border-radius: 8px; border: 1.5px solid #e2e8f0; font-size: 0.85rem; background-color: #f8fafc;">
            </div>
            <div class="col-md-4">
...
<div class="d-flex justify-content-start mb-3">
    <form action="<?php echo e(route('peminjaman.clearHistory')); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus semua riwayat peminjaman yang sudah dikembalikan? Data peminjaman yang masih aktif (Dipinjam) akan tetap disimpan.');">
        <?php echo csrf_field(); ?>
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    <div class="card-body px-0 py-0">
        <div class="d-flex justify-content-between align-items-center px-3 py-2 border-bottom" style="background: #f8fafc; border-radius: 12px 12px 0 0;">
            <div class="d-flex align-items-center gap-2">
                ...
                <select class="form-select form-select-sm" style="width: auto; min-width: 70px; border-radius: 6px; border: 1.5px solid #e2e8f0; font-size: 0.8rem; background-color: #fff;" onchange="window.location.href='<?php echo e(route('peminjaman.index')); ?>?per_page='+this.value+'&search=<?php echo e(request('search')); ?>&status=<?php echo e(request('status')); ?>'">
            ...
            <div class="d-flex gap-2">
                <a href="<?php echo e(route('peminjaman.exportExcel', request()->all())); ?>" class="btn btn-success btn-sm" style="background: linear-gradient(135deg, #10b981, #059669); border: none; font-weight: 600; border-radius: 8px; box-shadow: 0 2px 6px rgba(16, 185, 129, 0.25);">
                    <i class="fas fa-file-excel me-1"></i> Export Excel
                </a>
                <a href="<?php echo e(route('peminjaman.exportPDF', request()->all())); ?>" class="btn btn-danger btn-sm" style="background: linear-gradient(135deg, #ef4444, #dc2626); border: none; font-weight: 600; border-radius: 8px; box-shadow: 0 2px 6px rgba(239, 68, 68, 0.25);">
        ...
        <div class="table-responsive">
            <table class="table is-table mb-0">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $borrowings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $borrowing): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        <td class="ps-3 text-muted"><?php echo e($borrowings->firstItem() + $key); ?></td>
                        <?php if($borrowing->archive): ?>
                        <td class="fw-bold" style="font-size: 0.8rem;"><?php echo e($borrowing->archive->nomor_surat ?? '—'); ?></td>
                        <td><?php echo e($borrowing->archive->nama_arsip ?? '—'); ?></td>
                        <td style="font-size: 0.8rem;"><?php echo e($borrowing->archive->jenisArsip->nama ?? '—'); ?></td>
                        ...
                        <td style="font-size: 0.8rem;"><?php echo e($borrowing->divisi_peminjam ?? '—'); ?></td>
                        <td style="white-space: nowrap; font-size: 0.8rem;"><?php echo e($borrowing->tanggal_keluar ? \Carbon\Carbon::parse($borrowing->tanggal_keluar)->format('d-m-Y') : '—'); ?></td>
                        <td style="white-space: nowrap; font-size: 0.8rem;"><?php echo e($borrowing->tanggal_masuk ? \Carbon\Carbon::parse($borrowing->tanggal_masuk)->format('d-m-Y') : '—'); ?></td>
                        <td><span class="is-badge <?php echo e($borrowing->status_pinjam === 'Dipinjam' ? 'bg-danger' : 'bg-success'); ?>"><?php echo e($borrowing->status_pinjam); ?></span></td>
                    </tr>
    ...
    <div class="card-body border-top d-flex justify-content-between align-items-center" style="background: #f8fafc; border-radius: 0 0 12px 12px;">
        ...
        <?php echo e($borrowings->withQueryString()->links('pagination::simple-bootstrap-4')); ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\iron-smart\resources\views/peminjaman/daftar.blade.php ENDPATH**/ ?>