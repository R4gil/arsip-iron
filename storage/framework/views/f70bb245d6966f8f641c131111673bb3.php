...
<?php echo $__env->make('partials.page-header', ['title' => 'Manajemen Pengguna', 'subtitle' => 'Kelola akun pengguna dan hak akses sistem IRON SMART.', 'action' => route('pengguna.create'), 'actionLabel' => 'Tambah Pengguna'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    <div class="card-body px-0 py-0">
        ...
        <div class="d-flex justify-content-between align-items-center px-3 py-2 border-bottom" style="background: #f8fafc; border-radius: 12px 12px 0 0;">
            <div class="d-flex align-items-center gap-2">
                ...
                <select class="form-select form-select-sm" style="width: auto; min-width: 70px; border-radius: 6px; border: 1.5px solid #e2e8f0; font-size: 0.8rem; background-color: #fff;"
                    onchange="window.location.href='<?php echo e(route('pengguna.index')); ?>?per_page='+this.value+'&search=<?php echo e(request('search')); ?>&role=<?php echo e(request('role')); ?>'">
            ...
            <div class="d-flex gap-2">
                <a href="<?php echo e(route('pengguna.exportExcel', request()->query())); ?>" class="btn btn-success btn-sm" style="background: linear-gradient(135deg, #10b981, #059669); border: none; font-weight: 600; border-radius: 8px; box-shadow: 0 2px 6px rgba(16, 185, 129, 0.25);">
                    <i class="fas fa-file-excel me-1"></i> Export Excel
                </a>
                <a href="<?php echo e(route('pengguna.exportPDF', request()->query())); ?>" class="btn btn-danger btn-sm" style="background: linear-gradient(135deg, #ef4444, #dc2626); border: none; font-weight: 600; border-radius: 8px; box-shadow: 0 2px 6px rgba(239, 68, 68, 0.25);">
        ...
        <div class="px-3 py-2 border-bottom" style="background: #fff;">
            <form method="GET" action="<?php echo e(route('pengguna.index')); ?>" class="d-flex gap-2 flex-wrap align-items-center">
                <input type="hidden" name="per_page" value="<?php echo e(request('per_page', 10)); ?>">
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" class="form-control form-control-sm" placeholder="Cari nama, username, atau email..." style="max-width: 280px; border-radius: 8px; border: 1.5px solid #e2e8f0;">
                <select name="role" class="form-select form-select-sm" style="max-width: 160px; border-radius: 8px; border: 1.5px solid #e2e8f0;">
                    ...
                    <option value="pengguna" <?php echo e(request('role') === 'pengguna' ? 'selected' : ''); ?>>Pengguna</option>
                </select>
                ...
                <a href="<?php echo e(route('pengguna.index')); ?>" class="btn btn-sm" style="background: #f1f5f9; color: #64748b; border: 1.5px solid #e2e8f0; font-weight: 600; border-radius: 8px;">
        ...
        <div class="table-responsive">
            <table class="table is-table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-3 text-muted"><?php echo e($users->firstItem() + $key); ?></td>
                        <td>
                            <?php if($user->profile_photo): ?>
                                <img src="<?php echo e(route('profile.photo', $user->profile_photo)); ?>" alt="Foto" style="width: 36px; height: 36px; object-fit: cover; border-radius: 50%; border: 2px solid #d4af37;">
                            <?php else: ?>
                        ...
                        <td class="fw-bold"><?php echo e($user->nama_pengguna); ?></td>
                        <td><?php echo e($user->username); ?></td>
                        <td style="font-size: 0.8rem;"><?php echo e($user->unit_kerja ?: '—'); ?></td>
                        <td style="font-size: 0.8rem;"><?php echo e($user->email); ?></td>
                        <td>
                            <?php
                            ...
                            ?>
                            <span style="display:inline-block;padding:2px 10px;border-radius:20px;font-size:0.75rem;font-weight:700;<?php echo e($roleStyle); ?>">
                                <?php echo e(ucfirst($user->role)); ?>

                            </span>
                        </td>
                        <td style="white-space: nowrap; font-size: 0.8rem;"><?php echo e($user->created_at->format('d M Y')); ?></td>
                        <td class="text-center" style="white-space: nowrap;">
                            <a href="<?php echo e(route('pengguna.edit', $user->id)); ?>" class="btn btn-sm me-1" style="background:#fffbeb;color:#b45309;border:1.5px solid #fcd34d;font-weight:600;border-radius:8px;">Edit</a>
                            <?php if($user->id !== auth()->id()): ?>
                            <form method="POST" action="<?php echo e(route('pengguna.destroy', $user->id)); ?>" class="d-inline-block" onsubmit="return confirm('Yakin ingin menghapus pengguna ini?');">
    ...
    <div class="card-body border-top d-flex justify-content-between align-items-center" style="background: #f8fafc; border-radius: 0 0 12px 12px;">
        ...
        <?php echo e($users->withQueryString()->links('pagination::simple-bootstrap-4')); ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\iron-smart\resources\views/pengguna/daftar.blade.php ENDPATH**/ ?>