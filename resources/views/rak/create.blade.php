...
@section('content')
@include('partials.page-header', ['title' => 'Tambah Rak', 'subtitle' => 'Tambahkan rak ke lemari pada lokasi tertentu.'])
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    <div class="card-body p-4">
        <h6 class="fw-bold mb-4" style="color: #1e293b; font-size: 0.9rem; letter-spacing: 0.03em; text-transform: uppercase; border-bottom: 2px solid #d4af37; padding-bottom: 10px;">
            <i class="fas fa-layer-group me-2" style="color: #d4af37;"></i>Form Rak Baru
        </h6>
        <form action="{{ route('rak.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="ruangarsip_id" id="location_select" class="form-select" required
                        ...
                        @foreach($locations as $location)
                            <option value="{{ $location->id }}" {{ old('ruangarsip_id', $selectedLocation ?? '') == $location->id ? 'selected' : '' }}>{{ $location->ruangan }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="rak_nama" class="form-control" value="{{ old('rak_nama') }}" placeholder="Contoh: Rak 1" required
                        style="border-radius: 8px; border: 1.5px solid #e2e8f0; padding: 0.6rem 0.75rem; font-size: 0.9rem; background-color: #f8fafc;">
                </div>
                <div class="col-12">
                    <label class="form-label fw-semibold" style="color: #334155; font-size: 0.85rem;">Keterangan</label>
                    <textarea name="rak_keterangan" class="form-control" rows="2" placeholder="Deskripsi rak (opsional)"
                        style="border-radius: 8px; border: 1.5px solid #e2e8f0; padding: 0.6rem 0.75rem; font-size: 0.9rem; background-color: #f8fafc;">{{ old('rak_keterangan') }}</textarea>
                </div>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const selectedCabinet = '{{ old('lemari_id', $selectedCabinet ?? '') }}';
