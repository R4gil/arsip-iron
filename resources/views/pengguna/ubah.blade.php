...
@section('content')
@include('partials.page-header', ['title' => 'Edit Pengguna', 'subtitle' => 'Perbarui data dan role akses pengguna.'])
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    <div class="card-body p-4">
        <h6 class="fw-bold mb-4" style="color: #1e293b; font-size: 0.9rem; letter-spacing: 0.03em; text-transform: uppercase; border-bottom: 2px solid #d4af37; padding-bottom: 10px;">
            <i class="fas fa-user-edit me-2" style="color: #d4af37;"></i>Form Edit Pengguna
        </h6>
        <form method="POST" action="{{ route('pengguna.update', $user) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row g-3">
                <div class="col-md-6">
                    <label class="form-label fw-semibold" style="color: #334155; font-size: 0.85rem;">Nama Pengguna <span class="text-danger">*</span></label>
                    <input type="text" name="nama_pengguna" value="{{ old('nama_pengguna', $user->nama_pengguna ?? '') }}" class="form-control" required
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="unit_kerja" value="{{ old('unit_kerja', $user->unit_kerja) }}" class="form-control" required
                        style="border-radius: 8px; border: 1.5px solid #e2e8f0; padding: 0.6rem 0.75rem; font-size: 0.9rem; background-color: #f8fafc;">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <label class="form-label fw-semibold" style="color: #334155; font-size: 0.85rem;">Password Baru <span class="text-muted" style="font-weight: 400; font-size: 0.75rem;">(Kosongkan jika tidak diubah)</span></label>
                    <input type="password" name="password" class="form-control" placeholder="Min. 6 karakter"
                        style="border-radius: 8px; border: 1.5px solid #e2e8f0; padding: 0.6rem 0.75rem; font-size: 0.9rem; background-color: #f8fafc;">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-semibold" style="color: #334155; font-size: 0.85rem;">Konfirmasi Password</label>
                ...
                <div class="col-md-6">
                    ...
                    @if($user->profile_photo)
                        <div class="mt-2">
                            ...
                            <img src="{{ route('profile.photo', $user->profile_photo) }}" alt="Current Photo" style="width: 64px; height: 64px; object-fit: cover; border-radius: 50%; border: 2px solid #d4af37;">
                        </div>
                    @endif
                </div>
            </div>
