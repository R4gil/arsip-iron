...
@section('content')
@include('partials.page-header', ['title' => 'Tambah Lemari', 'subtitle' => 'Tambahkan lemari ke lokasi ruangan yang sudah ada.'])
...
<div class="card border-0 shadow-sm" style="border-radius: 12px;">
    <div class="card-body p-4">
        <h6 class="fw-bold mb-4" style="color: #1e293b; font-size: 0.9rem; letter-spacing: 0.03em; text-transform: uppercase; border-bottom: 2px solid #d4af37; padding-bottom: 10px;">
            <i class="fas fa-boxes me-2" style="color: #d4af37;"></i>Form Lemari Baru
        </h6>
        <form action="{{ route('lemari.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="ruangarsip_id" class="form-select" required
                        ...
                        @foreach($locations as $location)
                            <option value="{{ $location->id }}" {{ old('ruangarsip_id', $selectedLocation ?? '') == $location->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="lemari_nama" class="form-control" value="{{ old('lemari_nama') }}" placeholder="Contoh: Lemari A1" required
                        style="border-radius: 8px; border: 1.5px solid #e2e8f0; padding: 0.6rem 0.75rem; font-size: 0.9rem; background-color: #f8fafc;">
                </div>
                <div class="col-12">
                    <label class="form-label fw-semibold" style="color: #334155; font-size: 0.85rem;">Keterangan</label>
                    <textarea name="lemari_keterangan" class="form-control" rows="2" placeholder="Deskripsi lemari (opsional)"
                        style="border-radius: 8px; border: 1.5px solid #e2e8f0; padding: 0.6rem 0.75rem; font-size: 0.9rem; background-color: #f8fafc;">{{ old('lemari_keterangan') }}</textarea>
                </div>
            </div>
            <div class="d-flex gap-2 justify-content-end mt-4 pt-3 border-top">
                ...
                <button type="submit" class="btn fw-bold px-4 py-2" style="border-radius: 8px; background: linear-gradient(135deg, #d4af37, #aa7c11); color: #1e293b; border: none; box-shadow: 0 3px 10px rgba(212, 175, 55, 0.3);">
                    <i class="fas fa-save me-2"></i>Simpan Lemari
                </button>
            </div>
        </form>
    </div>
